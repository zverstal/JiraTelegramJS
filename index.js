...
function parseCustomMarkdown(text) {
    ...
    // Обрабатываем жирный текст (*жирный*)
    text = text.replace(/\*(.*?)\*/g, '<b>$1</b>');

    // Обрабатываем курсив (_курсив_)
    text = text.replace(/_(.*?)_/g, '<i>$1</i>');

    // Обрабатываем зачеркнутый текст (~зачеркнутый~)
    text = text.replace(/~(.*?)~/g, '<s>$1</s>');

    // Обрабатываем инлайн-код (`код`)
    text = text.replace(/`([^`]+)`/g, '<code>$1</code>');

    // Обрабатываем маркерные списки (- элемент, * элемент)
    text = text.replace(/^\s*[\-\*]\s+(.*)/gm, '• $1');

    // Обрабатываем нумерованные списки (1. элемент)
    text = text.replace(/^\s*\d+\.\s+(.*)/gm, '🔹 $1');

    // Обрабатываем таблицы, превращая их в моноширинный текст
